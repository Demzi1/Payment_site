...
const FeatureCard = ({title,  content, icon, index})=>(
  <div className={`flex flex-row justify-center items-center p-6 rounded-[20px] ${index ==! features.length - 1? 'mb-6' : 'mb-0'} feature-card`}>
    <div className={`w-[64px] h-[64px] rounded-full ${styles.flexCenter} bg-dimBlue`}>
      <img src={icon} className='w-[50px] h-[50px] object-fit'/>
    </div>
    ...
    <section id='features' className={`${layout.section}`}>
      <div className={`${layout.sectionInfo}`}>
        ...
        <p className={`${styles.paragraph} max-w-[470px] mt-5`}>
          with the right credit card, you can  improve you financial by building credit,
